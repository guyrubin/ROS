...
3. **AM-NEW-0 — Landing rebuild (draft autonomous; publish L3).** `arbor-content` rewrites the convergence section EN **+ native HE** to the six surfaces (incl. Practice Studio, Arbor Care, **and the kids-Academy hero world**); `arbor-design` builds the six surface cards. **Accept:** EN+HE parity, native-HE reviewer pass, honest framing (Care = warm-handoff, Masterclasses = "in production"), Arbor Bar pass. **Gate:** ECD + safety + native-HE review → **L3 publish/deploy = Guy** (it's the live owned web surface). *EN one-liner already corrected; this is the full section + HE.*
