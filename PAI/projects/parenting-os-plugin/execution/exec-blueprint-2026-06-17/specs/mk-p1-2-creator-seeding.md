...
- **Create:** `PAI/projects/parenting-os-plugin/marketing/arbor-creator-seeding-wave1.md` — the wave-1 seeding playbook (cohort, budget table, brief, link/UTM convention, run sheet, measurement loop). This is the primary deliverable and avoids touching the shared backlog.
- **Edit:** `PAI/projects/parenting-os-plugin/marketing/arbor-il-creator-tracker.md` — extend the tracker table (lines 57-63) with the wave-1 run-sheet columns (Wave, Fee, Post type, Post URL, Posted date, Installs, Activations, CPA) and pre-fill the wave-1 cohort rows with their `il-<slug>` codes + canonical links.
- **Edit (shared, append-only):** `PAI/projects/parenting-os-plugin/marketing/arbor-marketing-backlog.md` — flip P1-2 status note to reference the new playbook doc. Single-line append; see conflict notes.
