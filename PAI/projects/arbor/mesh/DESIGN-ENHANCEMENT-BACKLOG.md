...
- **[4|M] ✅ FIXED 2026-06-26** **Bedtime Stories orphan** — was fully-built (backend endpoint + tab) with no nav entry; now wired as a sidebar/mobile nav item under the Stories group (`navigation.ts`: `{ tab: "bedtime-stories", label: "Bedtime Story", icon: Moon }`). The capability stays safety-gated (AP-057: escalation-gated, generate-and-discard, redaction-enforced) — nav entry only, no gating change. [navigation.ts]
- **[4|M] ✅ FIXED 2026-06-26** **Mobile can't launch Kid Mode** — was desktop-only (`Topbar` hidden `md:flex`); a compact `<KidModeButton>` now lives in the in-content mobile accessories row (mirrors the compact `<AskArborButton>`), so mobile users reach Kid Mode. Desktop Topbar keeps the full pill. [Topbar.tsx, Shell.tsx]
