...
> **Marketing owns vs. hands off (the two-backlog rule).** This is the marketing team's **own** work queue. **Marketing owns and executes:** brand/positioning, campaigns, SEO/AEO, the **marketing landing pages + the owned web surface** (`html/arbor-marketing-landing-page-*.html`, `app/public/marketing/`), content + creative production, organic social, and the creator/community **distribution** engine — the whole viral content engine (§8, §9b/§9c). **Marketing does NOT build product code.** When a marketing goal needs a product change, marketing writes a **feature-request and hands it to the product backlog** ([`mesh/PRODUCT-BACKLOG.md`](../../PRODUCT-BACKLOG.md)) — it stays the *product* team's to build (the FR-1…FR-14 handoff in §9a). The same item is never tracked live in both backlogs.
...
| :-- | :-- | :-- | :-- |
| U1 | **Brand domain** | G-1 — WhatsApp OG previews + deep-link authority | **LIVE 2026-06-22 — arborparentingapp.com now resolves to the real Arbor app** (Firebase custom domain wired + Firebase Auth authorized domain added; login-breaks-on-new-domain fix applied; billing rails live per session memory). GoDaddy squatter / "Parenting Made Simple" page is gone. Left: (a) end-to-end test purchase via RevenueCat link; (b) kill-gate review — confirm `llms.txt` / `sitemap.xml` / `robots.txt` Sitemap line deploy to new canonical (see CIL-market-llms-txt-wrong-host, pending Firebase push); (c) push Frank Ruhl Libre HE font fix to prod. |
...
Net-new organic search needs 6–12 weeks of crawl trust — the 30-day SEO job is **the OG/share layer that makes WhatsApp shares convert + landing conversion**, not traffic. AEO answer-cards (HE-first, no IL competitor has zero HE content) are seeded now for a Week 5–6 payoff via AI answer engines (faster to index than Google). **Domain blocker is resolved (2026-06-22): arborparentingapp.com is now LIVE.** AEO hub can ship as soon as `llms.txt`, `sitemap.xml`, and `robots.txt` Sitemap line are pushed to prod — direct edits are done in workspace (CIL-market-llms-txt-wrong-host, status DONE pending deploy); `enrich-marketing-seo.mjs` run is not needed for this pass. Push these files + the HE font fix in a single Firebase deploy, then AEO publishing is unblocked.
...
| NOW-11 | Domain purchase (`arborparentingapp.com` acquired) + Firebase HTTPS + canonical switch | arbor-acquisition → Guy | was gated; now resolved | **DONE 2026-06-22 — domain LIVE serving the real Arbor app** (Firebase custom domain configured, Auth authorized domain added, login fix applied). Remaining: push `llms.txt`/`sitemap.xml`/`robots.txt` + HE font fix to prod; run end-to-end billing test. | viral-gtm + execution-tracker |
...
| **7.65** | **CIL-market-llms-txt-wrong-host** | llms.txt, sitemap.xml, robots.txt Sitemap line all carry arborprd-westeu.web.app — AEO citations point at uncitable Firebase host. 27 refs in llms.txt, 23 in sitemap. | arbor-seo | safe | **DONE (2026-06-22) — all 3 files swept to arborparentingapp.com in workspace. PENDING: Firebase deploy to make live. `enrich-marketing-seo.mjs` run not needed; direct edits applied.** |
...
| **[40→resolved]** | **CIL-market-no-brand-domain** | Canonical was `arborprd-westeu.web.app` — unshareable; gated entire GTM loop. | arbor-marketing-lead | resolved | **DONE 2026-06-22 — arborparentingapp.com is LIVE serving the real Arbor app.** Firebase custom domain wired; Firebase Auth authorized domain added; GoDaddy "Parenting Made Simple" page is gone. Marketing assets may now link to arborparentingapp.com. Next: push SEO file updates + HE font fix to prod; run billing end-to-end test. §6 DNS action item retired. |
...
| :-- | :-- | :-- | :-- |
| ~~**arborparentingapp.com DNS fix**~~ — **DONE 2026-06-22.** Domain is LIVE on the real Arbor app. Firebase custom domain wired; Auth authorized domain added; GoDaddy squatter page gone. Remaining: push `llms.txt`/sitemap/robots.txt + HE font fix to prod; end-to-end billing test (RevenueCat). Retired as a gated item. | — | resolved | — |
...
- `PAI/projects/arbor/mesh/improvement/IMPROVEMENT-BACKLOG.md` → folded into this backlog (CIL-market-* funnel findings §3; capability bets §5; shipped commits recorded)
- `PAI/projects/arbor/marketing/arbor-viral-gtm-2026-H2.md` → folded into this backlog (€10k budget allocation §6; market sequencing §4; brand platform §1)
- `PAI/projects/arbor/marketing/arbor-marketing-backlog-v2.md` → folded into this backlog (P0–P2 items §4; kill gates §4; adversarial stress-test gaps incorporated)
- `PAI/projects/arbor/marketing/arbor-essence-and-viral-brief.md` → folded into this backlog (moat one-liner §1; viral artifact table §5; brand DNA anti-references inform §4 copy gates)
- `PAI/projects/arbor/marketing/arbor-playkit-viral-production-pack-2026-06-20.md` → folded into this backlog (produced assets §2 NOW-1; HE headline options and video scripts in NOW-5; creator brief rules inform §4 P1-2)
- `PAI/projects/arbor/arbor-competitive-analysis-and-feature-defs-2026-06-14.md` → folded into this backlog (8-competitor map informs §5 feature table; Rhythm/Ask a Specialist/Daily Play feature defs handed to product)
- `PAI/projects/arbor/PRDs/PRD_2026-06-21_arbor-viral-capable-transforming.md` → folded into this backlog (VIRAL/CAPABLE/TRANSFORMING/TRUST north stars §1; G1/G2 governance gates §1; success metrics inform kill gates §4)
- `PAI/projects/arbor/marketing/arbor-30-day-subscription-blitz.md` → folded into this backlog (registration model math; "child is the angle, parent is the customer" spine; pricing ladder)
- `PAI/projects/arbor/marketing/arbor-loop-eng-spec.md` → folded into this backlog (P0-2/P0-3/P0-4 eng specs; analytics event schema §2 NOW-7)
- `PAI/projects/arbor/marketing/arbor-youtube-strategy.md` → folded into this backlog (Shorts now, long-form Phase 2+; zero new production cost in ignition §4)
- `PAI/projects/arbor/marketing/arbor-il-creator-tracker.md` → folded into this backlog (sourcing method + Tier A/B candidate list; verify before outreach §2 NOW-3)
- `PAI/projects/arbor/marketing/EXECUTION-TRACKER.md` → folded into this backlog (milestone status; blocked-on-Guy items §2 NOW-10/11/12)
- `PAI/projects/arbor/marketing/arbor-launch-copy-pack.md` → folded into this backlog (HE + EN hero headline options; #ArborAvatar challenge; DM outreach copy)
- `PAI/projects/arbor/marketing/arbor-marketing-backlog.md` → superseded by backlog-v2, which is superseded by this file
...
**9c. Execution plan — clear instructions (do in this order).** This is what the next `/arbor-marketing-loop` (or an on-demand push) executes. Each step names the owner pod, the concrete action, the acceptance bar, and the gate. Autonomous items publish to owned organic surfaces after the ECD + `arbor-safety` gate; **L3 = drafted, stops for Guy**; product-gated items wait on the named FR.

1. **AM-NEW-1 — Convergence carousel (DO FIRST, autonomous).** `arbor-content` drafts a 7-panel script HE-native + EN (panels 1–6 = each incumbent + its price + the one thing it can't do without the record; panel 7 = "one bill instead of the stack; one record none of them can become"). → `arbor-creative` builds it in Canva to the brand kit. **Accept:** 7 panels, no clinical/effect-size claim, no child data, passes the Arbor Bar (one-word-swap fails on a competitor). **Gate:** ECD + `arbor-safety` → publish to IG/TikTok owned queue (T2). *Establishes the organism so every later piece reads as proof.*
2. **AM-NEW-9 — Kid-hero angle (autonomous draft).** `arbor-content` + `arbor-creative` draft the "your child is the hero — and you get the comic" Story-Journey creative using a **stylized hero avatar (never a real face)**. **Accept:** kid-hero language, parent-mediated (parent shares), zero real-child media. **Gate:** ECD + safety → owned queue. *The share-loop itself is product-gated on **FR-14** — the creative angle ships now; the auto-share mechanic ships when FR-14 lands.*
3. **AM-NEW-0 — Landing rebuild (marketing-owned; draft autonomous; publish L3).** The landing pages are **marketing's surface**: `arbor-content` rewrites the convergence section EN **+ native HE** to the six surfaces (incl. Practice Studio, Arbor Care, **and the kids-Academy hero world**); `arbor-creative` builds the six surface cards/visuals; `arbor-seo` updates OG/schema/hreflang. **Accept:** EN+HE parity, native-HE reviewer pass, honest framing (Care = warm-handoff, Masterclasses = "in production"), Arbor Bar pass. **Gate:** ECD + safety + native-HE review → **L3 publish/deploy = Guy** (it's the live owned web surface). *EN one-liner already corrected; this is the full section + HE.*
4. **AM-NEW-2 — Rhythm "I was right" share card** (autonomous, W2): `arbor-content` caption pack HE+EN + `arbor-creative` static card. Ships now; the in-product auto-trigger needs **FR-1**.
5. **AM-NEW-4 / AM-NEW-5 / AM-NEW-8** (autonomous, P1): Daily Play "why Arbor picked this," Ask Arbor cited-answer card, Arbor Care warm-handoff explainer — each ECD + safety gated; AM-NEW-8 **must not** imply a live specialist marketplace.
6. **HELD until product lands:** AM-NEW-3 + AM-NEW-9 share mechanic (FR-14 / share-export P0-3) · AM-NEW-6 baby track (FR-7) · AM-NEW-7 "practice writes to the record" (FR-10). Keep the creative drafted; ship the moment the FR closes.

**Owner of this plan:** `arbor-marketing-lead`. **Standing instruction to `arbor-insights`:** refresh [CAPABILITY-MAP.md](CAPABILITY-MAP.md) each cycle and re-confirm the deployment gates before any held item ships.

---

### 10. Localization & Transcreation workstream (marketing-owned) — 2026-06-22

**Trigger (Guy):** `arbor-content`'s Hebrew read **translated, not native** — below the bar. Root fix: copy is now **message-first** and non-English markets go through **`arbor-localization`** (transcreation, never literal), gated by a **native-voice gate** ([LOCALIZATION.md](LOCALIZATION.md)). New pod + standard added; this workstream re-does HE to native quality and scales to the rollout languages. Marketing-owned (own backlog); no product handoff.
