...
| `arbor-content` | Content / copy (message-first) | The **master message** in the two source voices (EN + native HE) — landing copy, blog/guide-hub, share-card copy, lifecycle email; **writes the hook/caption first** (copy leads, creative serves). Concept-led, never functional/literal | `arbor-marketing-lead` |
| `arbor-localization` | Transcreation & Localization | Native, market-specific copy across the locale matrix (HE + the 5 rollout markets); **transcreates — never literal-translates**; owns the **native-voice gate** + RTL/i18n; blocks translated-feeling copy. Standard: [LOCALIZATION.md](LOCALIZATION.md) | `arbor-marketing-lead` |
...
**The org flow (a full viral growth org, not a copy team):** `arbor-insights` (what's working now) → `arbor-brand` + `arbor-content` (strategy + the master message) → `arbor-localization` (native transcreation per market) → `arbor-creative` (the watchable asset) → `arbor-distribution` (creators + community seeding + the queue) → `arbor-acquisition` (the loop + the read) → `arbor-critic-market` (eval) → back to `arbor-insights`. The method, cadence, and thresholds live in [VIRAL-ENGINE.md](VIRAL-ENGINE.md); the per-market native-copy standard in [LOCALIZATION.md](LOCALIZATION.md).
...
- **Native, not translated (every non-English asset)** — clears `arbor-localization`'s **native-voice gate** ([LOCALIZATION.md](LOCALIZATION.md)): transcreated not literal, no calques, message-first, correct local register + RTL; publish-bound copy flagged for a native-human reviewer. **Translated-feeling copy does not ship.**
...
- Saves under `PAI/projects/arbor/mesh/marketing/` and `marketing/`. Landing/web surface lives in the app at `app/landing/*.html` + `app/public/marketing/`.
