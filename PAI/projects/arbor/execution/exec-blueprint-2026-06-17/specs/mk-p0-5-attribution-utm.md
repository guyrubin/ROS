...
**Edit:**
- `PAI/projects/arbor/html/arbor-marketing-landing-page-en.html` — add `data-cta="app"` + static `href="https://arborprd-westeu.web.app/"` to the 3 conversion CTAs (lines ~147, ~158, ~286; 286 removes `onclick="return false"`); add the `appHandoffHref()` + binding script next to the existing smooth-scroll IIFE.
- `PAI/projects/arbor/html/arbor-marketing-landing-page-he.html` — same for the HE conversion CTAs (~997, ~1012, ~1506, ~1517, ~1528, ~1543; 1543 removes `onclick="return false"`); add the same script (RTL-safe, no layout change).
...
- `PAI/projects/arbor/marketing/arbor-launch-copy-pack.md` (or `arbor-marketing-backlog.md`) — paste the UTM scheme table as the canonical reference for link-builders.
