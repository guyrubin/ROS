...
PAI root holds only routing (`CLAUDE.md`) + memory (`MEMORY.md`). The company machine — mesh, marketing, QA, PRDs, templates — lives under `projects/arbor/` (Arbor = product company #1). Look there, not at PAI root, for the product org.
...
Arbor is now the group's **product company #1**, with its own company charter at `projects/arbor/COMPANY.md` (PAI remains its product owner; CoS/group is portfolio owner). See `/00_System/companies.md`.
