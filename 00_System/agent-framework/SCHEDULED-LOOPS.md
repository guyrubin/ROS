...
| `arbor-auto-promote` | CoS Delivery / Arbor | `0 * * * *` (HOURLY) | Promote the newest CI-built Ready Cloud Run revision (sitting at 0% traffic) to 100% prod after a live `GET / → 200` smoke; idempotent; closes the unreliable GitHub Actions `promote` job. Script: `release-engineering/auto-promote-arbor.sh` | **Prod-promote (Level-3)** — promotes traffic, but only a CI-green-gated revision, never flips a claim/feature flag | ✅ live (hourly) — `scheduled-tasks` MCP. ⚠️ **See efficiency note below** |

**⚠️ Efficiency + posture note on `arbor-auto-promote` (flagged 2026-06-23):** it wakes a full agent session every hour to run an idempotent bash script that usually no-ops — wasteful. It should run as a plain **Hermes cron / Task Scheduler** job invoking the `.sh` directly (no agent), and/or at a lower frequency. Its smoke is liveness-only (`GET /`), not a functional/claim check. Both are **Guy-gated** changes (prod-promotion cadence + runtime) — surfaced, not auto-changed.
